...
class ActorRolloutRefWorker(Worker):
    ...
    def load_checkpoint(self, path, del_local_after_load=False):
        ...
        self.checkpoint_manager.load_checkpoint(
            path=path, del_local_after_load=del_local_after_load
        )
...
class SolverModelWorker(Worker):
    """
    This is a solver model to solve the generate a solution for the genrated question which is used in calculating the rule
    based reward score.
    """
    ...
    def _build_model(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            warnings.simplefilter("ignore")
            # setattr(model_config, "classifier_dropout", 0.0)
            solver_module = AutoModelForCausalLM.from_pretrained(
                pretrained_model_name_or_path=local_path,
                config=model_config,
                torch_dtype=torch.bfloat16,
                attn_implementation="flash_attention_2",
            ...
            solver_module.to(torch.bfloat16)
        auto_wrap_policy = get_fsdp_wrap_policy(
            module=solver_module, config=self.config.model.fsdp_config
        )
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def generate_solution(self, prompts: DataProto):
        import itertools
        from verl.utils.seqlen_balancing import rearrange_micro_batches, get_reverse_idx
        ...
        with self.rollout_sharding_manager:
            ...
            if self._is_param_offload:
                offload_fsdp_model_to_cpu(self.solver_module)
            ...
            log_gpu_memory_usage("After generating solutions", logger=logger)
