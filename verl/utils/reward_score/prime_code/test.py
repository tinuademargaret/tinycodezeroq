...
def get_runtime_module(module_code):

    runtime_module = types.ModuleType("runtime_module")
    exec(module_code, runtime_module.__dict__)
...
def run_test(in_outs, test=None, debug=False, timeout=15):
    ...
    if test is None:
        ...
        if which_type == CODE_TYPE.call_based:
            ...
            try:
                tmp_sol = get_runtime_module(sol)
...
def custom_compare_(output, ground_truth):
    ...
    if isinstance(output, list):
        output_1 = "\\n".join(output)
        print(output_1)
        if stripped_string_compare(output_1, ground_truth.strip("\\n")):
            return True
